...
public class BankServiceImpl implements BankService {
    ...
    public UserDto deposit(String username, BigDecimal amount, Currency currency) {
        ...
        RLock lock = acquireLock(walletLockKey(username, currency));
        try {
            ...
            TransactionHistory history = logHistory(user, TransactionType.DEPOSIT, currency, amount, newBalance, null,
                    amount + " " + currency + " yatırıldı");
    ...
    public UserDto withdraw(String username, BigDecimal amount, Currency currency) {
        ...
        try {
            ...
            TransactionHistory history = logHistory(user, TransactionType.WITHDRAW, currency, amount, newBalance, null,
                    amount + " " + currency + " çekildi");
            UserDto dto = convertToDto(user);
    ...
    @Transactional
    @Caching(evict = {
            @CacheEvict(value = "userInfo", key = "#fromUsername"),
            @CacheEvict(value = "userInfo", key = "#toUsername"),
            @CacheEvict(value = "userList", allEntries = true)
    })
    public UserDto transfer(String fromUsername, String toUsername, BigDecimal amount, Currency currency) {
        validateAmount(amount);
        ...
        User receiver = userRepository.findByUsername(toUsername)
                .orElseThrow(() -> new BankException("Hedef kullanıcı bulunamadı: " + toUsername));

        String keyA = walletLockKey(fromUsername, currency);
        String keyB = walletLockKey(toUsername, currency);
        ...
        try {
            ...
            BigDecimal senderNewBalance = senderWallet.getBalance().subtract(amount).setScale(2, RoundingMode.HALF_UP);
            BigDecimal receiverNewBalance = receiverWallet.getBalance().add(amount).setScale(2, RoundingMode.HALF_UP);
    ...
    public UserDto exchange(String username, Currency fromCurrency, Currency toCurrency, BigDecimal amount) {
        ...
        String keyA = walletLockKey(username, fromCurrency);
        String keyB = walletLockKey(username, toCurrency);
        String firstKey = keyA.compareTo(keyB) <= 0 ? keyA : keyB;
        String secondKey = keyA.compareTo(keyB) <= 0 ? keyB : keyA;
        ...
        try {
            ...
            BigDecimal fromNewBalance = fromWallet.getBalance().subtract(amount).setScale(2, RoundingMode.HALF_UP);
            BigDecimal toNewBalance = toWallet.getBalance().add(convertedAmount).setScale(2, RoundingMode.HALF_UP);
            ...
            TransactionHistory inHistory = logHistory(user, TransactionType.EXCHANGE_IN, toCurrency, convertedAmount,
                    toNewBalance, null, note);
